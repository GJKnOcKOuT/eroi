 ...
 *
 * @package    backend\modules\aster_partnership_profiles\widget\views
...
\arter\amos\tag\assets\ModuleTagFormAsset::register($this);
    ...
    <div id="<?= $id ?>" class="<?= $containerClass ?> body">
        <div class="intestazione-box">
            <?php if (!$hideHeader): ?>
                <?php
                if (!$is_search) {
                    echo \arter\amos\core\helpers\Html::tag('h3', AmosTag::tHtml('amostag', '#tags_title_tree'), [
                        'class' => 'tags-title'
                    ]);
                } else {
                    echo \arter\amos\core\helpers\Html::tag('label', AmosTag::tHtml('amostag', '#tags_label_tree'));
                }
                ?>
            <?php endif; ?>

            <?php
            $recipientsDescriptionTab['community'] = AmosTag::t('amostag', '#community_recipients_description');
            $recipientsDescriptionTab['news'] = AmosTag::t('amostag', '#news_recipients_description');
            $recipientsDescriptionTab['partnership-profiles'] = AmosTag::t('amostag', '#partnership-profiles_recipients_description');

            if(!empty($recipientsDescriptionTab[Yii::$app->controller->id])): ?>
                <div class="recipients">
                    <?= $recipientsDescriptionTab[Yii::$app->controller->id] ?><br /><br />
                </div>
            <?php endif; ?>
        </div>
        <?php
