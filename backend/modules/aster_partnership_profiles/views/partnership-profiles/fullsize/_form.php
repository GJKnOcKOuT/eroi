...
use arter\amos\partnershipprofiles\models\IntellectualProperty;
...
$expressionOfInterestDescriptionTab = Module::t('amospartnershipprofiles', '#expression_of_interest_other_informations');
$attachmentDescriptionTab = Module::t('amospartnershipprofiles', '#partnership_profiles_attachment_tab_description');
...
<?php echo WorkflowTransitionStateDescriptorWidget::widget([
    'form' => $form,
    'model' => $model,
...
<div class="<?= Yii::$app->controller->id ?>-form">
    ...
    <?php if (!empty($expressionOfInterestDescriptionTab)): ?>
        <div class="expression-of-interest tab-form-description">
            <?= $expressionOfInterestDescriptionTab ?>
        </div>
    <?php endif; ?>
        ...
        <?php if (!empty($enabledFields['development_stage_id']) && $enabledFields['development_stage_id'] == true) { ?>
            <div class="col-lg-6 col-sm-6">
                <?= $form->field($model, 'development_stage_id')->widget(Select::className(), [
                    'data' => ArrayUtility::translateArrayValues(PartnershipProfilesUtility::getDevelopmentStageListReadyForSelect(), 'amospartnershipprofiles'),
                    'options' => [
                        'lang' => substr(Yii::$app->language, 0, 2),
                        'multiple' => false,
                        'placeholder' => Module::t('amospartnershipprofiles', 'Select/Choose') . '...',
    ...
    <?php if (!empty($attachmentDescriptionTab)): ?>
        <div class="attachment tab-form-description">
            <?= $attachmentDescriptionTab ?>
        </div>
    <?php endif; ?>
    ...
    <div class="pubblication-request-tip">
        <?= Module::t('amospartnershipprofiles', '#partnership_profiles_pubblication_request_tip'); ?>
    </div>
    ...
    <!-- ?= CloseSaveButtonWidget::widget($closeSaveWidgetConf); ? -->

    <?php
    $statusToRenderToHide = $model->getStatusToRenderToHide();
    $workflowInitialStatusId = $model->getWorkflowSource()->getWorkflow(PartnershipProfiles::PARTNERSHIP_PROFILES_WORKFLOW)->getInitialStatusId();
    ?>
    ...
    <?= WorkflowTransitionButtonsWidget::widget([
        ...
        'workflowId' => PartnershipProfiles::PARTNERSHIP_PROFILES_WORKFLOW,
        'viewWidgetOnNewRecord' => true,
        'closeButton' => Html::a(Module::t('amospartnershipprofiles', 'Annulla'), $appController->getFormCloseUrl(), ['class' => 'btn btn-secondary']),
        ...
        'draftButtons' => [
            PartnershipProfiles::PARTNERSHIP_PROFILES_WORKFLOW_STATUS_TOVALIDATE => [
                ...
                'description' => 'le modifiche e mantieni la sfida in "richiesta di pubblicazione"'
            ],
