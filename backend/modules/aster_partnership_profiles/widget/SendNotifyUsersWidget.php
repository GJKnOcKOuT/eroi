 ...
 */
class SendNotifyUsersWidget extends Widget
{
    ...
    /**
     * @var string $checkList
     */
    private $checkList = 'tag'; 
...
//        }
    ...
    protected function renderContent()
    {
        ...
        if (PartnershipProfilesUtility::loggedUserIsAnimationOrCM()) {
            $createUrlParams = [
                '/partnershipprofiles/partnership-profiles/contatta-users-ajax',
            ];
            ...
            $messageSection =
                ...
                Html::tag('div',
                    Html::a(Module::tHtml('amoscore', '#close'), null, [
                        'class' => 'btn btn-secondary success-modal-'.$key.'-close-btn hidden',
                        'data-dismiss' => 'modal'
                    ]),
                    ['class' => 'm-15-0']
                );
            ...
            $baseModalContent =
                Html::hiddenInput('notify-api-ids', $this->model, ['id' => 'notify-api-'.$key.'-ids', 'class' => 'notify-api-id-class']) .
                Html::tag('div',
                ...
                Html::tag('div',
                    ...
                    Html::a(Module::t('partnershipprofiles', 'Invia'), $createUrlParams, ['class' => 'btn btn-navigation-primary send-notify-users-'.$key.'-modal-btn']),
            ...
            $btn = Html::a(Module::t('partnershipprofiles', 'Messaggio a più EROI'),
                '#',
                [
                    'id' => 'messaggio-a-eroi-'.$key,
                    'data-toggle' => 'modal',
                    'data-target' => '#' . 'amos-modal-'.$key.'-ids',
                    'title' => Module::t('partnershipprofiles', 'Messaggio a più EROI'),
        ...
        if ($this->autoRegisterJavascript) {
            static::registerWidgetJavascript($key, $this->checkList);
        }
    ...
    private function getMessage() {
        ...
        $message = Module::t('amospartnershipprofiles', "#contactUsers",
     ...
     */
    public static function registerWidgetJavascript($key, $checkList)
    {	$messageAlert = Module::t('amospartnershipprofiles', 'Seleziona EROI da contattare');
        $js = <<<JS
        ...
        $('.send-notify-users-{$key}-modal-btn').on('click', function (event) {
            event.preventDefault();
            
            var modal = $(this).parents('.modal-send-tmb-notify');
            var hiddenInput = modal.find('#notify-api-{$key}-ids','#amos-modal-{$key}-ids');
            var apiId = hiddenInput.val();
            
            var id = 'notify_textarea-{$key}-ids';
        ...
        jQuery('#messaggio-a-eroi-{$key}').on('click', function() {
            var selezionati = jQuery('.correlations-target-checkbox-{$checkList}:checked');
            var concat = [];
            if (selezionati.length == 0){
                alert('{$messageAlert}');
                return false;
            } else {
                selezionati.each(function() {
                        concat.push(jQuery(this).val());
                });
                var joinet = concat.join();
                jQuery('#notify-api-{$key}-ids','#amos-modal-{$key}-ids').val(concat.join());
            }
        });
JS;
