 ...
 *
 * @package    backend\modules\aster_core\forms
...
use backend\modules\aster_admin\widgets\UserCardWidget;
...
class ItemAndCardHeaderWidget extends Widget
{
     ...
     */
    public $layout = "@vendor/arter/amos-core/forms/views/widgets/item_and_card_header_widget.php";
    ...
    {
        $targetsCollection = \arter\amos\cwh\models\CwhNodi::findAll([
            'id' => $nodes
        ]);
        ...
        foreach ($targetsCollection as $target) {
            $targetString = "";
//            if(array_key_exists('arter\amos\community\models\CommunityContextInterface', class_implements($this->findNode($target)))){
//                $targetString .= Module::t('amoscore', 'Community') . ' ';
//            }
//            if(array_key_exists('arter\amos\core\interfaces\OrganizationsModelInterface', class_implements($this->findNode($target)))){
...
//            }
    ...
    {
        $validatorsCollection = \arter\amos\cwh\models\CwhNodi::findAll([
            'id' => $validators
        ]);
        ...
        foreach ($validatorsCollection as $singleValidator) {
            if (!(strpos($singleValidator->id, 'user') !== false)) {
                $targetString = "";
                if (array_key_exists('arter\amos\community\models\CommunityContextInterface', class_implements($this->findNode($singleValidator)))) {
                    $targetString .= Module::t('amoscore', '#item_card_header_widget_from_community') . ' ';
                }
                if (array_key_exists('arter\amos\core\interfaces\OrganizationsModelInterface', class_implements($this->findNode($singleValidator)))) {
    ...
    public static function getValidatorNameGeneral($validators)
    {
        ...
        foreach ($validatorsCollection as $singleValidator) {
            if (!(strpos($singleValidator->id, 'user') !== false)) {
                ...
                $targetString = "";
                if (array_key_exists('arter\amos\community\models\CommunityContextInterface', class_implements($model))) {
                    $targetString .= Module::t('amoscore', 'community') . ' ';
                }
                if (array_key_exists('arter\amos\core\interfaces\OrganizationsModelInterface', class_implements($model))) {
                    $targetString .= Module::t('amoscore', 'organizzazione') . ' ';
                }
