...
use arter\amos\layout\toolbar\Nav;
use arter\amos\layout\toolbar\NavBar;
...
//if there is information page for policy or cookies - display link at the end of user menu
    ...
    <header>
        ...
        if (!CurrentUser::getUserIdentity()) {
            ...
            /** @var \arter\amos\socialauth\Module $socialAuthModule */
...
//                ])
                ...
                'items' => [
                    ...
                    '<li><a href="/site/termini-uso" title="Termini d\'uso" tabindex="-1">Termini d\'uso</a></li>'
                ],
            ...
            if (\Yii::$app->controller instanceof \arter\amos\dashboard\controllers\base\DashboardController) {
            ...
            if (\Yii::$app->getModule('chat')) {
                $widget      = new \arter\amos\chat\widgets\icons\WidgetIconChat();
                ...
                $chatLink    = Html::tag('li',
                        Html::a(
                            AmosIcons::show('comments-o', [], 'dash')."<span class='badge'>".(($bulletCount > 0 ) ? $bulletCount
                                : "" )."</span>"
                            , '/messages',
                            ['title' => \arter\amos\chat\AmosChat::t('amoschat', 'Messaggi privati')]
                        ), ['class' => 'header-plugin-icon']
                );
                $menuItems[] = $chatLink;
            }
            ...
            if (\Yii::$app->getModule('myactivities') && !\Yii::$app->user->isGuest && Yii::$app->user->can('MYACTIVITIES_READ')) {
                $widget      = new \arter\amos\myactivities\widgets\icons\WidgetIconMyActivities();
                if (
                ...
                $myActivitiesLink    = Html::tag('li',
                        Html::a(
                            ...
                            ['title' => \arter\amos\myactivities\AmosMyActivities::t('amosmyactivities',
                                'My activities')]
                        ), ['class' => 'header-plugin-icon']
                );
