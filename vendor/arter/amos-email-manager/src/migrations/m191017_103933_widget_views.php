...
class m191017_103933_widget_views extends Migration
{
    ...
    private function setPermissionConfs()
    {
        $this->perms = array(
            array(
                'name' => \arter\amos\emailmanager\widgets\icons\WidgetIconViews::className(),
                'type' => '2',
    ...
    public function safeUp()
    {
        ...
        $this->batchInsert('{{%auth_item_child}}', ['parent', 'child'], [
            ['ADMIN', \arter\amos\emailmanager\widgets\icons\WidgetIconViews::className()],
        ]);
        ...
        $this->batchInsert('{{%amos_widgets}}', ['classname', 'type', 'module', 'status', 'child_of', 'created_by', 'created_at', 'updated_by', 'updated_at'], [
            [
                \arter\amos\emailmanager\widgets\icons\WidgetIconViews::className(),
                'ICON',
                $module,
                $status,
                'arter\amos\emailmanager\widgets\icons\WidgetIconEmailManager',
                $userId,
                $now,
                $userId,
                $now
