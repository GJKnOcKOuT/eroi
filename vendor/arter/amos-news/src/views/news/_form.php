    ...
    <div class="news-form col-xs-12 nop">
        ...
        <div class="row">
            ...
            if (!$model->isNewRecord && isset($moduleCwh)) {
                if (!empty($model->validatori)) {
                    foreach ($model->validatori as $validatore) {
                        $network = \arter\amos\cwh\utility\CwhUtil::getNetworkFromId($validatore);
                        if ($network instanceof \arter\amos\community\models\Community) {
            ...
            <?php if (\Yii::$app->getModule('correlations')) { ?>
                <?=
                arter\amos\correlations\widget\ManageCorrelationsButtonWidget::widget([
                    'model' => $model
                ]);
                ?>
                <?php
            ...
            if ($disableStandardWorkflow == false) {
                $draftButtons = [
                    ...
                    News::NEWS_WORKFLOW_STATUS_VALIDATO => [
                        'button' => \arter\amos\core\helpers\Html::submitButton(AmosNews::t('amosnews', 'Salva'),
