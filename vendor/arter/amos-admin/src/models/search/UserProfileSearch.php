...
use arter\amos\core\record\SearchResult;
use Yii;
use yii\base\Model;
...
class UserProfileSearch extends UserProfile implements SearchModelInterface
{
    ...
    public function searchCommunityManagerUsers($params)
    {
        ...
        if (!is_null($communityModule)) {
            ...
            $communityTableName       = \arter\amos\community\models\Community::tableName();
            $communityUserMmTableName = \arter\amos\community\models\CommunityUserMm::tableName();
            ...
            $queryAll->andWhere([
                $communityUserMmTableName.'.status' => \arter\amos\community\models\CommunityUserMm::STATUS_ACTIVE,
                $communityUserMmTableName.'.role' => \arter\amos\community\models\CommunityUserMm::ROLE_COMMUNITY_MANAGER,
            ]);
            $queryAll->andWhere([$communityUserMmTableName.'.deleted_at' => null]);
            $queryAll->andWhere([$communityTableName.'.deleted_at' => null]);
            $queryAll->andWhere(['<>', $communityTableName.'.community_type_id', \arter\amos\community\models\CommunityType::COMMUNITY_TYPE_CLOSED]);
            ...
            $managerQuery->innerJoin($communityUserMmTableName,
                $communityTableName.'.id = '.$communityUserMmTableName.'.community_id');
            $managerQuery->andWhere([$communityUserMmTableName.'.status' => \arter\amos\community\models\CommunityUserMm::STATUS_ACTIVE]);
            $managerQuery->andWhere([$communityTableName.'.context' => \arter\amos\community\models\Community::className()]);
            ...
            $managerQuery->andWhere(['<>', $communityTableName.'.community_type_id', \arter\amos\community\models\CommunityType::COMMUNITY_TYPE_CLOSED]);
