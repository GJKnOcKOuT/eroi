        ...
        [
            ...
            'checkboxOptions' => function ($model, $key, $index, $column) use ($availableUserProfileIds) {
                if (!in_array($model->id, (array)$availableUserProfileIds)) {
                    return [
                        'disabled' => true,
                        'title' => \arter\amos\admin\AmosAdmin::t('amosadmin', "Non è possibile condividere il contenuto con questo utente. Il contenuto si trova in una community non accessibile a questo utente")
                    ];
                }
                return '';
            }
...
<div class="col-xs-12 nop modal-contact-comment">
    <label class="control-label"><?= \arter\amos\admin\AmosAdmin::t('amosadmin', 'Comment') ?></label>
    <?= \arter\amos\core\helpers\Html::textarea(\arter\amos\admin\AmosAdmin::t('amosadmin', 'Comment'),'', ['id' => 'text-share', 'class' => 'form-control']); ?>
</div>


<?php echo \arter\amos\core\helpers\Html::button(\arter\amos\admin\AmosAdmin::t('amosadmin','Condividi'), [
