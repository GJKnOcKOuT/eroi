...
class UserDropController extends Controller
{
    ...
    public function dropMessages($userRecord) {
        $q = \arter\amos\chat\models\Message::find();
    ...
    public function dropCommunity($user){
        ...
        if($moduleCommunity){
            $communityUsers = \arter\amos\community\models\CommunityUserMm::find()->andWhere(['user_id' => $user->id])->all();
            /** @var  $communityUser \arter\amos\community\models\CommunityUserMm */
            foreach ($communityUsers as $communityUser){
                if(!$this->hasCommunityManagers($communityUser->community_id, $user)){
                    ...
                    if($this->isAdminCommunityParticipant($communityUser->community_id)){
                        $moduleCommunity->changeRoleCommunityUser($communityUser->community_id, $newCommManagerId, \arter\amos\community\models\CommunityUserMm::ROLE_COMMUNITY_MANAGER);
                    }
                    else {
                        $moduleCommunity->createCommunityUser($communityUser->community_id, \arter\amos\community\models\CommunityUserMm::STATUS_ACTIVE, \arter\amos\community\models\CommunityUserMm::ROLE_COMMUNITY_MANAGER, $newCommManagerId);
                    }
    ...
    public function hasCommunityManagers($community_id, $user){
        $countCm = \arter\amos\community\models\CommunityUserMm::find()
    ...
    public function isAdminCommunityParticipant($communityId){
        $userMm  =\arter\amos\community\models\CommunityUserMm::find()->andWhere(['community_id' => $communityId, 'user_id' => 1])->one();
        return !empty($userMm);
    }
