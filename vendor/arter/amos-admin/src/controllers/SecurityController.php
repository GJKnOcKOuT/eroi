...
use arter\amos\admin\models\ProfileReactivationForm;
use arter\amos\admin\models\RegisterForm;
use arter\amos\admin\models\TokenUsers;
...
//                return $this->redirect($url);
    ...
    public function actionLogout($goToFrontPage = false, $backTo = null)
    {
        ...
        if (isset(\Yii::$app->params['template-amos']) && \Yii::$app->params['template-amos']) {
            ...
            $ids      = \arter\amos\dashboard\models\AmosUserDashboards::find()->andWhere(['user_id' => $idUtente])->select('id');
            \arter\amos\dashboard\models\AmosUserDashboardsWidgetMm::deleteAll(['IN', 'amos_user_dashboards_id',
                $ids]);
            \arter\amos\dashboard\models\AmosUserDashboards::deleteAll(['user_id' => $idUtente]);
        }
        if($goToFrontPage) {
...
//            $postLoginUrl  = Yii::$app->getUrlManager()->createUrl(['/community/join', 'id' => $community_id]);
    ...
    protected function createSocialUser($userProfile, \Hybrid_User_Profile $socialProfile, $provider)
    {
        try {
            /**
             * @var $socialUser \arter\amos\socialauth\models\SocialAuthUsers
             */
            $socialUser = new \arter\amos\socialauth\models\SocialAuthUsers();
