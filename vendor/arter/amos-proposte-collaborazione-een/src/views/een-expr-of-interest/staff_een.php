 ...
 * */
$this->title = \arter\amos\een\AmosEen::t('amoseen', 'Staff EEN');
...
<?php
$form = \arter\amos\core\forms\ActiveForm::begin(['id' => 'form-staff-een']); ?>
...
?>
    ...
    <?php echo \arter\amos\core\views\AmosGridView::widget([
        ...
        'columns' => [
            ...
            'user.email',
            [
                'label' =>  \arter\amos\een\AmosEen::t('amoseen','#staff_default'),
                'value' => function($model)use (&$form) {
                    $staffMember = \arter\amos\een\models\EenStaff::find()->andWhere(['user_id' => $model->user_id])->one();
                    if(empty($staffMember)) {
                        $staffMember = new \arter\amos\een\models\EenStaff();
                    }
            ...
            [
                'label' => \arter\amos\een\AmosEen::t('amoseen','#network_node_EEN'),
                'value' => function($model)use (&$form) {
                    $staffMember = \arter\amos\een\models\EenStaff::find()->andWhere(['user_id' => $model->user_id])->one();
                    if(empty($staffMember)) {
                        $staffMember = new \arter\amos\een\models\EenStaff();
                    ...
                    return $form->field($staffMember, "[$model->user_id]een_network_node_id")->widget(\kartik\select2\Select2::className(),[
                        'data' => \yii\helpers\ArrayHelper::map(\arter\amos\een\models\base\EenNetworkNode::find()->all(),'id', 'name'),
                        'options' => ['placeholder' => \arter\amos\een\AmosEen::t('amoseen', 'Select...'), 'class' => 'required-field']
                    ])->label(false)
                ...
                'buttons' => [
                    'disassociate' => function ($url, $model) use($userStaffEenWithEOI){
                        if(!in_array($model->id, $userStaffEenWithEOI)) {
                            return \yii\helpers\Html::a(\arter\amos\core\icons\AmosIcons::show('delete'), null, [
    ...
    echo \arter\amos\core\helpers\Html::submitButton(\arter\amos\een\AmosEen::t('amoseen', '#save'), ['class' => 'btn btn-navigation-primary pull-right']);
