...
//        }
        ...
        if ($request_info == 0)  {
            $subject = AmosEen::t('amoseen',
                'Manifestazione di interesse su opportunità EEN ID {codice_een}',
                ['codice_een' => $modelEenExprOfInterest->eenPartnershipProposal->reference_external]);
    ...
    public static function sendEmailWorkflowSuspended($model)
    {
        ...
        if ($staffEen) {
            ...
            if ($model->is_request_more_info == 0) {
                $subject = AmosEen::t('amoseen', "La tua manifestazione di interesse {een_id} è in stato “sospeso”",
                        ['een_id' => $code]);
                $message = AmosEen::t('amoseen',
                ...
                $subject = AmosEen::t('amoseen', "La tua richiesta di informazione {een_id} è in stato “sospeso”",
                        ['een_id' => $code]);
                $message = AmosEen::t('amoseen',
...
//            pr($ex->getMessage());
    ...
    public static function sendEmailTagsNotFound()
    {
        try {
            ...
            $message = "<p>".AmosEen::t('amoseen',
                    "Dopo l'importazione delle Proposte EEN risultano dei tag non mappati.<br>Si può procedere al completamento della mappatura al seguente <a href=\"{url}\">link</a>",
                    ['url' => $url])."</p>";
