...
use arter\amos\documenti\widgets\DocumentsOwlCarouselWidget;
...
if ($foldersEnabled) {
    ...
    if (!isset(\Yii::$app->params['hideListsContentCreatorName']) || (\Yii::$app->params['hideListsContentCreatorName'] === false)) {
        $columns['updated_by'] = [
            ...
            'value' => function ($model) {
                $profile = \arter\amos\admin\models\UserProfile::find()->andWhere(['user_id' => $model->updated_by])->one();
                if (empty($profile)) {
                    return '';
                }
    ...
    'buttons' => [
        ...
        'update' => function ($url, $model) use ($enableVersioning) {
            ...
            if (Yii::$app->user->can('DOCUMENTI_UPDATE', ['model' => $model])) {
                ...
                return Html::a(\arter\amos\core\icons\AmosIcons::show('edit'), $action, $options);
            }
        }
    ]
];
...
echo DocumentsOwlCarouselWidget::widget([
    ...
    'singleItemView' => '@vendor/arter/amos-documenti/src/views/documenti/amos_owl_carousel_widget_item',
