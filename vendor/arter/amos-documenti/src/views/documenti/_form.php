...
/** @var \arter\amos\documenti\controllers\DocumentiController $appController */
...
/** @var \arter\amos\comments\AmosComments $commentsModule */
            ...
            <?php if (!$isFolder && $enableCategories): ?>
                <div class="col-md-6 col-xs-12">
                    <?= $form->field($model, 'documenti_categorie_id')->widget(Select2::className(), [
                        ...
                        'data' => ArrayHelper::map(\arter\amos\documenti\utility\DocumentsUtility::getDocumentiCategorie()->orderBy('titolo')->all(), 'id', 'titolo')
                    ]); ?>
                </div>
                <div class="col-md-6 col-xs-12">
        ...
        <?php if ($showReceiverSection): ?>
            ...
            <div class="col-xs-12">
                ...
                <div class="col-xs-12 receiver-section">
                    <?=
                    \arter\amos\cwh\widgets\DestinatariPlusTagWidget::widget([
                        'model' => $model,
                        'moduleCwh' => $moduleCwh,
                        'scope' => $scope
                    ]);
                ...
                if (!empty($moduleGroups) && !empty($moduleCommunity) && !empty($moduleCwh)) {
                    ...
                    if (isset($moduleCommunity)) {
                        $dataProvider = new \yii\data\ActiveDataProvider([
                            'query' => \arter\amos\groups\models\Groups::getGroupsByParent()
                        ]);
                    } else {
                        $dataProvider = new \yii\data\ActiveDataProvider([
                            'query' => \arter\amos\groups\models\Groups::find()->andWhere(0)
                        ]);
                    }

                    \yii\widgets\Pjax::begin(['id' => 'pjax-container', 'timeout' => 2000, 'clientOptions' => ['data-pjax-container' => 'grid-members']]);
                    $pjaxContent = \arter\amos\core\views\AmosGridView::widget([
