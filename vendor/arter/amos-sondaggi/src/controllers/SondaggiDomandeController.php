...
use arter\amos\sondaggi\models\search\SondaggiDomandeSearch;
 ...
 *
 * @property \arter\amos\sondaggi\models\SondaggiDomande $model
 * @property \arter\amos\sondaggi\models\search\SondaggiDomandeSearch $modelSearch
 *
 * @package arter\amos\sondaggi\controllers
 */
...
//        return parent::actionIndex($layout); // TODO sistemare questo punto cambiando totalmente la action in quanto non compatibile con gli standard di PHP 7
        ...
        if ($this->model->load(Yii::$app->request->post())) {
            ...
            if ($this->model->domanda_condizionata) {
                foreach ($condizioneNecessaria as $cond) {
                    $condizione                                   = new \arter\amos\sondaggi\models\SondaggiDomandeCondizionate();
            ...
            if ($this->model->modello_risposte_id) {
                 $num = \arter\amos\sondaggi\models\SondaggiRispostePredefinite::importFromModello($this->model->modello_risposte_id, $this->model->id);
            }
            if ($url) {
                $this->redirect($url);
            } else {
    ...
    public function actionUpdate($id, $url = null)
    {
        ...
        if ($this->model->load(Yii::$app->request->post()) && $this->model->validate()) {
            ...
            $this->model->setValidazione($this->model->validazione); 
            \arter\amos\sondaggi\models\SondaggiDomandeCondizionate::deleteAll(['sondaggi_domande_id' => $id]);
            if ($this->model->domanda_condizionata) {
                foreach ($condizioneNecessaria as $cond) {
                    $condizione                                   = new \arter\amos\sondaggi\models\SondaggiDomandeCondizionate();
                    $condizione->sondaggi_risposte_predefinite_id = $cond;
