...
use kartik\mpdf\Pdf;
use Yii;
use arter\amos\sondaggi\assets\ModuleRisultatiFrontendAsset;
...
use arter\amos\socialauth\controllers\SocialAuthController;
...
//                foreach ($domandeWithFilesModels as $doma
    ...
    protected function registerUser($sondaggioId, $idSessione)
    {
        ...
        if (!empty($model->mail_conf_community_id)) {
            $community = \arter\amos\community\models\Community::findOne($model->mail_conf_community_id);
        }
...
//         creo un nuovo utente
    ...
    protected function getCampiMappati($model, $idSessione)
    {
        ...
        $campi    = \arter\amos\sondaggi\models\SondaggiMap::find()->asArray()->all();
        $campiArr = [];
        foreach ($campi as $v) {
...
//        ])->one();
    ...
    {
        $preference = \arter\amos\translation\models\TranslationUserPreference::find()->andWhere(['user_id' => $user_id]);
        if ($preference->count()) {
            $model       = $preference->one();
            $model->lang = $language;
            $model->save(false);
        } else {
            $newPreference          = new \arter\amos\translation\models\TranslationUserPreference();
    ...
    public function registerToCommunity($community, $user)
    {
        if ($community) {
            ...
            if ($moduleCommunity) {
                $count = \arter\amos\community\models\CommunityUserMm::find()->andWhere(['user_id' => $user->id,
                        'community_id' => $community->id])->count();
                if ($count == 0) {
                    $moduleCommunity->createCommunityUser($community->id,
                        \arter\amos\community\models\CommunityUserMm::STATUS_ACTIVE,
                        \arter\amos\community\models\CommunityUserMm::ROLE_PARTICIPANT, $user->id);
                    return true;
                }
            }
        }
