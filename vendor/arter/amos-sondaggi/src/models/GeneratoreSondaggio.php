...
class GeneratoreSondaggio extends \yii\base\Model {
	 ...
	 *
	 * @param arter\amos\sondaggi\models\SondaggiDomandePagine $pagina
	 *
		...
		foreach ( $domande->all() as $Domanda ) {
			...
			*/
			...
			if ( $Domanda->abilita_ordinamento_risposte ) {
				$sposasu  = \arter\amos\sondaggi\AmosSondaggi::t( 'amossondaggi', 'Sposta su' );
				$sposagiu = \arter\amos\sondaggi\AmosSondaggi::t( 'amossondaggi', 'Sposta giu' );
    ...
    public function generaRuleMinMax($domanda, $min = null, $max = null)
    {
        $minMaxRule = "";
        if (!empty($min) && !empty($max)) {
            $minMaxRule = "[['".$domanda."'], 'arter\\amos\\sondaggi\\validators\\Cardinality', 'min' => $min, 'max' => $max]";
        } else if (!empty($min) && empty($max)) {
            $minMaxRule = "[['".$domanda."'], 'arter\\amos\\sondaggi\\validators\\Cardinality', 'min' => $min]";
        } else if (empty($min) && !empty($max)) {
            $minMaxRule = "[['".$domanda."'], 'arter\\amos\\sondaggi\\validators\\Cardinality', 'max' => $max]";
        }
