...
class TagWidgetAreeInteresse extends \yii\widgets\InputWidget
{
        ...
        if (isset($moduleTag)) {
            ...
            foreach ($contents as $content) {
                ...
                $query = \arter\amos\tag\models\Tag::find()
                    ...
                    ->andWhere([
                        \arter\amos\tag\models\TagModelsAuthItemsMm::tableName() . '.classname' => $content,
    ...
    /**
     * @param $tagId
     * @return \arter\amos\tag\models\Tag
     */
    ...
    private function fetchRoles()
    {
        ...
        if (isset($moduleTag)) {
            /**@var ActiveQuery $query * */
            $query = \arter\amos\tag\models\Tag::find()->joinWith('cwhTagInterestMm')->andWhere(['auth_item' => array_keys(\Yii::$app->authManager->getRolesByUser(\Yii::$app->getUser()->getId()))]);
