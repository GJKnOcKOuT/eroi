...
namespace arter\amos\cwh\utility;

use arter\amos\admin\AmosAdmin;
use arter\amos\core\record\Record;
use arter\amos\cwh\AmosCwh;
use arter\amos\cwh\base\ModelNetworkInterface;
use arter\amos\cwh\exceptions\CwhException;
...
/**
 * Class CwhUtil
 * @package arter\amos\cwh\utility
 */
class CwhUtil
{
    /**
    ...
    public static function getTagNames($tagIds)
    {
        ...
        if (isset($moduleTag) && !empty($tagIds)) {
            if (is_array($tagIds)) {
                $tags = ArrayHelper::map(\arter\amos\tag\models\Tag::find()->andWhere([
                    'in',
                    'id',
                    $tagIds
                ])->all(),
                ...
                $tags = ArrayHelper::map(\arter\amos\tag\models\Tag::find()->andWhere('id in (' . $tagIds . ')')->all(),
                    'id', 'nome');
            }
     ...
     * @return \arter\amos\cwh\models\query\CwhRegolePubblicazioneQuery|mixed $publicationRulesQuery
     */
     ...
     * @param \arter\amos\tag\models\Tag $organizationTag
     * @param int $userProfileId
    ...
    {
        if (!($organizationTag instanceof \arter\amos\tag\models\Tag)) {
            throw new CwhException('Param organizationTag must be an instance of \arter\amos\tag\models\Tag');
        }
