                ...
                if ($this->widgets_type == AmosWidgets::TYPE_ICON) {
                    ...
                    if ($widgets->count()) {
                        foreach ($widgets->all() as $value) {
                            if ($value->classname != 'arter\amos\admin\widgets\icons\WidgetIconUserProfile' &&
                                $value->classname != 'arter\amos\community\widgets\icons\WidgetIconCommunityDashboard' &&
                                (
                                empty(\Yii::$app->params['isPoi']) || (!empty(\Yii::$app->params['isPoi']) && \Yii::$app->params['isPoi']
                                == false) || $this->model->className() != '\arter\amos\community\models\Community'
                                || ($value->classname != '\arter\amos\projectmanagement\widgets\icons\WidgetIconprojects'
                                /* && $value->classname !=  '\arter\amos\documenti\widgets\icons\WidgetIconDocumentiDashboard' */)
                                || ( $value->classname == '\arter\amos\projectmanagement\widgets\icons\WidgetIconprojects'
    ...
    protected function runHtml($widgets)
    {
        ...
        if ($widgets->count()) {
            if ($this->graphicCustomSize) {
                foreach ($widgets->all() as $value) {
                    if($this->model->className() != '\arter\amos\community\models\Community'
                                   || ($value->classname !=  '\arter\amos\documenti\widgets\graphics\WidgetGraphicsUltimiDocumenti')
                                    || ($value->classname ==  '\arter\amos\documenti\widgets\graphics\WidgetGraphicsUltimiDocumenti'
                                           && ($this->model->id != 2751)))
                            {
