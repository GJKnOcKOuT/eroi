...
use arter\amos\chat\widgets\ConversationWidget;
use arter\amos\chat\widgets\MessageWidget;
use arter\amos\chat\widgets\UserContactsWidget;
...
if($showVideoConference) {
    $loggedUserProfileId = \arter\amos\admin\models\UserProfile::find()->andWhere(['user_id' => \Yii::$app->user->id])->one()->id;
    $receiverUserProfileId = null;
    if(!is_null($contact)){
        $receiverUserProfile = \arter\amos\admin\models\UserProfile::find()->andWhere(['user_id' => $contact->id])->one();
