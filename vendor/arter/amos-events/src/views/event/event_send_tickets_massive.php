                ...
                'downloadTickets' => function ($url, $model) use ($event) {
                    ...
                    if ($event->has_tickets) {
                        ...
                        if (!empty($model)) {
                            ...
                            $userCommunityMm = \arter\amos\community\models\CommunityUserMm::find()->andWhere(['user_id' => \Yii::$app->user->id])->andWhere([
                                    'community_id' => $event->getCommunityModel()->id])->one();
                            if (\arter\amos\events\utility\EventsUtility::checkManager($event)) {
                                $btn = Html::a(
                                        AmosIcons::show('download', ['class' => '']), $url,
                                        ['class' => 'btn btn-tools-primary', 'title' => \arter\amos\events\AmosEvents::txt('Download Tickets'),
