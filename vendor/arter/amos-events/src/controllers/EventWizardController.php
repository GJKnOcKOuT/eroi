...
use arter\amos\events\components\PartsWizardEventCreation;
 ...
 *
 * @property \arter\amos\events\models\Event $model
 *
 * @package arter\amos\events\controllers
 */
class EventWizardController extends CrudController
{
    ...
    private function detachCwhBehavior()
    {
        ...
        if (isset($cwhModule) && in_array($this->eventsModule->model('Event'), $cwhModule->modelsEnabled)) {
            $this->model->detachBehaviorByClassName(\arter\amos\cwh\behaviors\CwhNetworkBehaviors::className());
        }
    }
}
