...
class EventsUtility
{
    ...
    public static function duplicateEventTagForCommunity($model)
    {
        ...
        if (isset($moduleTag) && in_array($eventsModule->model('Event'),
            ...
            $eventTags = \arter\amos\tag\models\EntitysTagsMm::findAll([
                'classname' => $eventsModule->model('Event'),
                'record_id' => $model->id
            ]);
            foreach ($eventTags as $eventTag) {
                $entityTag = new \arter\amos\tag\models\EntitysTagsMm();
    ...
    public static function getUserCalendarService($userId = null)
    {
        ...
        if (!is_null($socialAuth)) {
            ...
            $socialAuthUser = \arter\amos\socialauth\models\SocialAuthUsers::findOne([
    ...
    /**
     * @param null|\arter\amos\socialauth\models\SocialAuthServices $service
    ...
    public static function isLoggedUserPartner($event_calendars_id){
        $count = \arter\amos\events\models\EventCalendars::find()
