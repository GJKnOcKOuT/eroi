 ...
 * CommunitySearch represents the model behind the search form about `arter\amos\community\models\Community`.
 * @package arter\amos\community\models\search
 */
class CommunitySearch extends Community implements SearchModelInterface, CmsModelInterface
{
    ...
    public function searchCommunitiesRecommended($params, $limit = null)
    {
        ...
        $query->andWhere(['in', self::tableName().'.context', [Community::className(), 'arter\amos\events\models\Event']]);
        ...
        if (!isset($params[$this->formName()]['tagValues'])) {
            $loggedProfile = \arter\amos\admin\models\UserProfile::find()->andWhere(['user_id' => \Yii::$app->user->id])->one();
