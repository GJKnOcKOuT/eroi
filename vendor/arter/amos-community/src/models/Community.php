...
use arter\amos\community\i18n\grammar\CommunityGrammar;
...
use arter\amos\community\widgets\UserNetworkWidget;
use arter\amos\core\user\User;
use arter\amos\cwh\AmosCwh;
use arter\amos\cwh\behaviors\TaggableInterestingBehavior;
 ...
 *
 * @property \arter\amos\community\models\CommunityUserMm[] $communityManagerMms
 * @property \arter\amos\core\user\User[] $communityManagers
 *
 * @package arter\amos\community\models
 */
class Community extends \arter\amos\community\models\base\Community implements CommunityContextInterface
{
...
//    return $ret;
    ...
    public function getCommunityManagerMms()
    {
        return $this->getCommunityUserMms()
            ->andWhere([\arter\amos\community\models\CommunityUserMm::tableName() . '.status' => \arter\amos\community\models\CommunityUserMm::STATUS_ACTIVE])
            ->andWhere([\arter\amos\community\models\CommunityUserMm::tableName() . '.role' => \arter\amos\community\models\CommunityUserMm::ROLE_COMMUNITY_MANAGER]);
    }
    ...
    {
        return $this->hasMany(\arter\amos\core\user\User::className(), ['id' => 'user_id'])->via('communityManagerMms');
    }
    ...
    public function showParticipantWidget(){
        if($this->getAmosWidgetsIcons()->count() > 0) {
            $count = $this->getAmosWidgetsIcons()->andWhere(['classname' => 'arter\amos\admin\widgets\icons\WidgetIconUserProfile'])->count();
    ...
    public function showSubCommunityWidget(){
        if($this->getAmosWidgetsIcons()->count() > 0) {
            $count = $this->getAmosWidgetsIcons()->andWhere(['classname' => 'arter\amos\community\widgets\icons\WidgetIconCommunityDashboard'])->count();
            return $count > 0;
        }
        return true;
    }
