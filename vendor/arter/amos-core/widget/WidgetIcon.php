...
class WidgetIcon extends WidgetAbstract
{
    ...
    public function getHtml()
    {
        ...
        if (!empty($moduleL)) {
            $assetBundle = \arter\amos\layout\assets\BaseAsset::register($controller->getView());
        } else {
            $assetBundle = \arter\amos\core\views\assets\AmosCoreAsset::register($controller->getView());
        }

        $view = '@vendor/arter/amos-core/widget/views/icon';
        if ($this->getEngine() == WidgetAbstract::ENGINE_ROWS) {
            $view = '@vendor/arter/amos-core/widget/views/icon_rows';
        }
