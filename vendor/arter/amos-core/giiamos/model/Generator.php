 ...
 *
 * @package    arter\amos\core\giiamos\model
...
//        pr('$baseClassWrapper'); die;
     ...
     * 
     * [
     *  [isCommentable] => arter\amos\comments\models\CommentInterface,
     * ]
     * 
    ...
    protected function getMethods($baseclssel)
    {
        ...
        foreach ($cls->getMethods() as $m) {
            try {
                if (!$cls->isInterface()) {
                    if ($m->class !== $baseclssel) {
                        if (strpos($m->class, 'arter\amos') === 0 && strpos($m->class, 'interface') !== false) {
