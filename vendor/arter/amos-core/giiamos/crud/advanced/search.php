...
//    pr($generator->getFormTabsAsArray());die();
...
foreach ((array) $generator->getMmRelations() as $Relation):
    if (Inflector::id2camel($Relation['fromEntity'], '_') == $modelClass):
        ...
        if (!(in_array($attributoEntita, $arrayAttributi))):
            ...
            $fields = [];
            foreach ($Relation['toFields'] as $key => $field) {
                if($key == 'toFieldName'){
                    $fields[] = $field;
                }
            ...
            if (count($fields) >= 1) {
                ...
                $condizioniAggiuntive[] = "\$query->andFilterWhere(['like', $newExpr, \$this->" . Inflector::id2camel($attributoEntita, '_') . "])";
            }

            $otherRules[] = "['" . Inflector::id2camel($attributoEntita, '_') . "', 'safe']";
            ...
            if (count($fields) >= 1) {
                ...
                $condizioniAggiuntive[] = "\$query->andFilterWhere(['like', $newExpr, \$this->" . Inflector::id2camel($newAttributoEntita, '_') . "])";
            }
            $otherRules[] = "['" . Inflector::id2camel($newAttributoEntita, '_') . "', 'safe']";
