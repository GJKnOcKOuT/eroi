...
use arter\amos\core\behaviors\EJsonBehavior;
use arter\amos\core\behaviors\SoftDeleteByBehavior;
use arter\amos\core\behaviors\VersionableBehaviour;
use arter\amos\core\interfaces\CrudModelInterface;
 ...
 * Class Record
 *
 * @property \arter\amos\admin\models\UserProfile $createdUserProfile
 * @property \arter\amos\admin\models\UserProfile $updatedUserProfile
 * @property \arter\amos\admin\models\UserProfile $deletedUserProfile
        ...
        if ($this->isEnabledCwh($cwhModule)) {
            if ($this->isEnabledTag()) {
                if (!$cwhModule->tagsMatchEachTree) {
                    ...
                    $tagRoots = \arter\amos\tag\models\TagModelsAuthItemsMm::find()->andWhere(['classname' => $this->className()])->addSelect('tag_id')->groupBy('tag_id')->column();
                    if ($tagRoots) {
                        $ruleTags = [
                            [
                                'tagsMandatory',
    ...
    public function getValidatorUsersId()
    {
        ...
        try {
            ...
            if ($this->isEnabledCwh($moduleCwh)) {
                $users = \arter\amos\cwh\models\CwhAuthAssignment::find()->andWhere([
    ...
    private function updateFrequentlyCommunityUpdate()
    {
        ...
        if (isset($moduleCommunity) && isset($moduleCwh) && !empty($moduleCwh->getCwhScope())) {
            ...
            if (isset($scope['community'])) {
                $id        = $scope['community'];
                $community = \arter\amos\community\models\Community::findOne($id);
                if (!empty($community)) {
