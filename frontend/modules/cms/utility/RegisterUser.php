...
use arter\amos\core\record\RecordDynamicModel;
use arter\amos\core\user\User;
use arter\amos\events\controllers\EventController;
use arter\amos\events\models\Event;
use arter\amos\events\models\EventGroupReferentMm;
...
//            $model->save();
        ...
        if ($community) {
            ...
            if ($moduleCommunity) {
                ...
                if ($count == 0) {
                    ...
                    if ($context == 'arter\amos\events\models\Event') {
    ...
    public function sendMail($model, $data, $waiting, $user = null, $isWaiting = null)
    {
        ...
        if($event){
            ...
            if($eventTemplates){
                ...
                $subject = \arter\amos\events\utility\EventMailUtility::parseEmailWithParams($event, $user->id, $subject, false);
                $text = \arter\amos\events\utility\EventMailUtility::parseEmailWithParams($event, $user->id, $text);
                if(!empty($linkToken)) {
    ...
    private function baseSendMail($model, $data, $message, $mailup = false)
    {
        ...
        if (isset($mailModule)) {
            ...
            $tos = [$model->$toField];
            if($mailup){
                $result = \arter\amos\events\utility\EventMailUtility::sendEmailTest($from, $tos, $data['email_subject'], $message);
            }else {
